...
**消耗层（Counter Layer）**：根据用户的生日、家人年龄、见面频率等自动计算"你和父母还能共度约 552 小时（约 92 次见面）"这种数字。每张时间账户卡片上展示。

**存储层（Deposit Layer）**：用户主动把有意义的时刻（一次旅行、一顿饭、一个生日）手动"存进"对应时间账户。带标题 + 长文字 + 图片（只从系统 PhotosPicker 选入，V1 不拍摄）。这些瞬间**永久保留，不会被消耗层扣减**。展示为"已存入 56h · 12 个瞬间"。

两层在每张时间账户卡片同时对照展示 —— 一个在减少，一个在增加。这就是 App 叫"时间银行"的核心：有流出、有存入、有账户余额。

## 6 个内置时间账户

陪父母（珊瑚橘）/ 陪孩子（金黄）/ 陪伴侣（薰衣草紫）/ 创造（晨雾蓝）/ 运动（薄荷绿）/ 自由时间（蜜桃色）

每个时间账户独立展示双数字（消耗 vs 存储）+ 副文案（如"约 92 次见面"、"黄金期剩 7.2 年"）。

## Memorial Mode（关键情感机制）

任何关系时间账户（父母 / 孩子 / 伴侣）可以被标记为"纪念模式"—— 用于承接"家人离世"的场景。
...
- 每个时间账户 icon 是小插画（不是符号）：两人剪影 / 牵手 / 对坐等
...
- 底部三按钮（横向等分）：**[编辑][换时间账户][删除]**

---

## 关键补充 8 屏（必画）

### Screen 7 · Memorial Mode 时间账户详情
- 同 Screen 4 布局，但**移除消耗层**（不显示"552h 约 92 次见面"和参数卡）
- 顶部插画保留温暖（一个远山剪影或光晕）
- 大数字只显示"**56h**"
- 副标题 chip："**致爸爸**"（带心形 icon + 时间账户名）
...
- **变体 C · 最近瞬间**：小时间账户 icon + "最近：杭州西湖 48h · 3 天前"
...
- Frame 800-1000ms · 光点飞向右上角对应时间账户卡
