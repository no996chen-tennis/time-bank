...
> 本文件是 Codex Project 会自动加载的项目索引。当前 PRD 版本：**V1.3.4**（2026-05-09 · WidgetKit 实装与 UI 优化）
...
**做：** Onboarding（4 步 + chip 多选，支持单身/无父母/丁克） · 6 内置时间账户双数字卡片 · 时刻 CRUD（图文，只从相册选）· Memorial Mode（任何关系时间账户）· 账户 Tab · 锁屏 Rectangular Widget + 桌面 Medium Widget · Raw ZIP 导出 · **仅 Light Mode**（无 Dark Mode 规划）

**推迟到 V1.1+：** 今日此刻 · 自定义时间账户 CRUD · 分享卡 · 视频拍摄 · 保存到相册 · 桌面 Widget 大号 · StandBy · 多格式导出 · 从 ZIP 恢复 · CloudKit 同步（V1.5 两阶段做）
...
## 设计方向（待 Figma 出图）
- 方向 A：Codex 风（干净、大留白、serif 字体、米白色调）
...
- `PRD-时间银行-V1.md` — **V1.3.4** 产品需求文档（含 §7.6 Schema + §21 Formatter Matrix + §22 主页 layout 硬约束 + WidgetKit snapshot 规则）
- `Use-Cases-详尽交互.md` — **V1.2** 所有模块 CRUD 逐步交互（V1 工程交付真相源）
- `Use-Cases-全版本梳理.md` — **V1 + V1.1 + V1.5 + V2+** 全版本 UC 索引（high-level · 漏洞自检用）
- `测试-M3端到端用例.md` — Codex computer use 端到端测试脚本
- `ChatGPT-协作协议.md` — ChatGPT 写代码 + Codex review 的协同工作流（**ChatGPT 第一个读**）
- `Claude-新窗口启动prompt.md` — 新 Claude Code 窗口启动 prompt
- `designs/DesignTokens.swift` — Swift 常量（**当前实现引用** · 待 V1 UI 重设计后替换）
...
- `文案系统.md` — **V1.3** Tone of voice + 文案库 + Widget/通知双池 + Formatter Matrix
