...
注：UC-0.2 设置补充家人信息 原属 M2，V1.3.2 + Adam 2026-04-24 决策挪到 M3 合并做（跟 UC-2.3 时间账户参数编辑共用设置页 UI 架构，效率更高）。

### M3 · 时间账户详情 + 参数编辑 + 设置 → 个人信息（第 6 周）
**交付：**
- UC-2.1 / 2.2 / 2.3（不做 2.4-2.7）
- UC-2.3 所有内置时间账户的参数滑块
- UC-0.2 设置补充/修改家人信息（从 M2 移入）
...
- 统一账户下沉协议（Moment 迁 `other` + `originDimensionId`）
