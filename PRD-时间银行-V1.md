...
> - 账户 Tab 升 P0；"今日此刻"、自定义时间账户 CRUD、分享卡、视频拍摄、保存到相册、桌面 Widget、StandBy、多格式导出、从 ZIP 恢复——全部推迟到 V1.1+（桌面 Medium 已在 V1.3.3 拉回 V1，桌面大号仍后置）
> - CloudKit 同步拆两阶段（metadata → media）
> - 通知默认文案改为中性，关系型提醒改为 opt-in
>
> **V1.3.1 补丁（2026-04-22 晚 · Claude Design 首版 review 后）：**
> - **新增「时间余额」内置顶部时间账户**（`Dimension.id = "lifespan"`）：主单位用**周**（呼应《四千周》），副文案 "N 年 · N 万小时"
...
>
> **V1.3.3 补丁（2026-05-07 · 今年视角 + Widget + 媒体体验修正）：**
> - 新增「今生 / 今年」时间窗口：主页顶部、时间账户卡片、详情页和时间公式页可切换；今年视角展示截至当年 12 月 31 日的余额。
...
Widget              · 锁屏 Rectangular + 桌面 Medium + 基础时间账户偏好
...
4. **Widget / StandBy**：V1 只保锁屏 Rectangular + 桌面 Medium 两个固定尺寸；桌面大号与 StandBy 继续后置。Dark Mode 短期无规划（产品策略上认为奶咖暖色 Light 是核心调性的载体）。
...
- 选择视频后必须立即显示导入中状态；若原文件拷贝或缩略图生成需要 5-10 秒，界面需持续展示 ProgressView / loading tile，不能空白等待。
- 存入流程 ≤ 30 秒（熟手场景）
...
- 视频封面不得使用低分辨率时间线缩略图放大；详情页需从原视频生成高分辨率封面，或使用不低于 720px 长边的缩略图。
- 图片与视频在新增/编辑时刻页使用同一 1:1 媒体格规格，不能因为媒体类型不同导致卡片大小不一致。
...
- 时间账户进入 memorial 后：
  - 数字区只保留"已存入 56 小时 · 12 个瞬间"，不再显示剩余小时 / 副文案
  - 卡片下方显示 chip："致爸爸"
  - 时间账户色保留（不变灰、不变冷）
...
| P0-13 | Widget（锁屏 Rectangular + 桌面 Medium） | 锁屏展示「今年余额 X 周」+ 核心时间账户；桌面 Medium 展示今年余额、3 个时间账户余额、`已存入 N 个瞬间`；从 App 生成的预计算快照读取（走 App Group）|
...
| **P0-21** | **时间余额（lifespan）顶部时间账户** | **内置 `Dimension.id = "lifespan"` (`kind = systemTop`)；主页顶部双层卡上半展示 `约 N 周` + 副 `N 年 · N 万小时`；预期寿命默认 85 可在设置调** |
...
| P1-3 | 桌面 Widget 大号 | Home Screen Widget 大号多时间账户 grid |
...
为什么不全改周：陪父母 "3.3 周"（552 小时 ÷ 168 小时）数学对但直觉怪——用户实际每年见 4 次 × 6 小时，这不是"3 周"能描述的感受。**顶部时间余额走周（作为哲学 anchor），其他时间账户走小时（作为具体场景 anchor）**，这种"刻意不一致"是设计选择不是 bug。

**副文案用「次」「年」作人类尺度翻译**——"552 小时"对普通人抽象，"约 92 次见面"立刻可想象。

**存储层量词用「瞬间」（不用"次"）**——避免与消耗层的"次"混淆；一个瞬间可能是 2 小时也可能是 48 小时，强调"被感受的片段"而非"打卡"。
...
- **主显示**：`约 2,704 周` · **副文案**：`45 年 · 47.3 万小时`
...
- **主显示**：`552 小时` · **副文案**：`约 92 次见面`

**陪孩子：**
- 输入：孩子年龄
- 高质量陪伴时间按年龄递减：0-5 岁 30 小时/周、6-12 岁 20 小时/周、13-17 岁 10 小时/周、18+ 岁 2 小时/周
- 核心窗口：18 岁前
- **主显示**：`7,488 小时` · **副文案**：`黄金期剩 7.2 年`

**运动：**
- 按年龄段动态调整：50 岁前 5 小时/周、50-80 岁 3 小时/周、80+ 岁 1 小时/周
- **主显示**：`8,160 小时` · **副文案**：`每周约 5 小时`

**创造/工作：**
- 主要创造期：当前年龄到 65 岁
- 自由创造期：65-预期寿命
- 每周创造时间：主要期 40 小时/周（可调）、自由期 20 小时/周
- **主显示**：`4.6 万小时` · **副文案**：`黄金期剩 22 年`

**陪伴侣：**
- 每日共处时间：4 小时（默认值，可调）
- 计算基于自己或伴侣中较短的预期寿命
- **主显示**：`1.8 万小时` · **副文案**：`约共度 40 年`

**自由时间：**
- 总可支配时间（每天 16 小时清醒可支配时间）
- 减去以上所有时间账户的时间
- **主显示**：`17.6 万小时` · **副文案**：`约 20 年自由时间`

### 存储层展示规则

- **单卡片底部**："已存入 56 小时 · 12 个瞬间"（小时为主，瞬间数为辅）
- **总账户（主页顶部）**："已存入 128 小时 · 27 个瞬间 · 跨 5 个时间账户"
- **时刻详情页**：发生时长 `48 小时` · 媒体数 `5 个媒体`

### 大数字可读性规则

| 范围 | 格式 |
|------|------|
| < 10,000 | `552 小时`、`7,488 小时`（千分位） |
| 10,000 – 999,999 | `1.8 万小时`（一位小数万小时）|
| ≥ 1,000,000 | `100 万小时` |
...
| thumbnailPath | String? | 缩略图相对路径（由 App 生成的 JPG，长边默认不低于 720px；详情页视频封面可直接从原视频生成高分辨率帧）|
| fileSize | Int64 | 字节数 |
...
@Model final class Dimension {
    ...
    var iconKey: String
    var colorKey: String                         // rose / warm / lavender / sage / sky / peach / coral / mint / denim / mauve
    var sortIndex: Int
...
struct FreeDimensionParams: Codable {
    var awakeHoursPerDay: Double          // 默认 16 · 每天清醒小时
}

enum CustomFormula: String, Codable {
    case weeklyHours                       // 每周 N 小时
    case dailyHours                        // 每天 N 小时
    case occurrenceBased                   // 每年 N 次 × 每次 M 小时
}

struct CustomDimensionParams: Codable {
    var formula: CustomFormula             // 默认 weeklyHours
    var weeklyHours: Double                // 默认 5 · formula == weeklyHours 时使用
    var dailyHours: Double                 // 默认 1 · formula == dailyHours 时使用
    var annualOccurrences: Int             // 默认 12 · formula == occurrenceBased 时使用
    var hoursPerOccurrence: Double         // 默认 2 · formula == occurrenceBased 时使用
}
...
Lifespan.remainingHoursK  = Lifespan.remainingYears * 365.25 * 24 / 1000   // 工程仍保留 hoursK，展示时转为「万小时」

// 今年视角
YearScope.remainingYears  = daysBetween(now, Dec 31 23:59:59 of current year) / 365.25
YearScope.remainingWeeks  = floor(daysBetween(now, Dec 31 23:59:59 of current year) / 7)
YearScope.remainingHoursK = YearScope.remainingYears * 365.25 * 24 / 1000
```
...
- **「今生 / 今年」只影响消耗层余额口径**，不影响已存入小时、瞬间数、聚合范围或 Memorial mode。
- 用户从主页以「今年」进入某个时间账户详情时，详情页默认继承「今年」视角；详情页与时间公式页都提供「今生 / 今年」切换。
...
│       ├── 01.heic                        ← 原图
│       ├── 01.thumb.jpg                   ← 缩略图，长边默认不低于 720px
...
| Widget（锁屏 Rectangular + 桌面 Medium） | 第 10 周 | App Group 共享 snapshot → Widget 渲染 | Widget 是第二大风险点，只做 2 个固定尺寸，桌面大号继续后置 |
...
│   │   ├── 上半：时间余额（lifespan）· 约 N 周 / N 周 · N 年/N 万小时
│   │   └── 下半：已存入 · N 小时 · N 个瞬间 · 跨 M 个时间账户（可点进账户 Tab 总览）
...
│
├── [时间账户详情页]
│   ├── 顶部：「今生 / 今年」时间窗口切换（默认继承主页进入时状态）
│   ├── 顶部 — 消耗层（剩余小时 + 副文案）
│   ├── 中段 — 时间公式（可编辑参数 + 扣减节奏说明）
│   ├── 下段 — 存储层时间线（时刻卡片列表）
│   │   └── 点时刻 → [时刻详情页]
│   └── 右上「+」→ [新增时刻表单]（预填当前时间账户）
│
...
- [ ] Widget 在真机锁屏 Rectangular 与桌面 Medium 均可显示（桌面大号、StandBy V1 不做）
...
| R4 | "预期寿命"数字触发用户情感不适（尤其父母） | 低 | 高 | 文案绝不说"父母还剩 X 年"；改成"你们还能共度 552 小时" + 副文案"约 92 次见面" |
...
|----------|------|---------|---------|
| `hoursCompact(h)` | 小时展示 | `552 小时`、`1,240 小时`、`1.8 万小时` | 卡片主数字、Widget |
| `hoursReadable(h)` | 小时完整 | `128 小时`、`48 小时` | 主页顶部总账户、时刻详情 chip |
| `hoursWithMinutes(s)` | 带分钟 | `2 小时 30 分钟`、`30 分钟` | 新增时刻表单时长滑块预览、参数编辑 |
| `hoursInDays(h)` | 小时 → 天数辅助显示 | `≈ 23 天`、`≈ 414 天`、`≈ 2,967 天` | **V1.3.2 新增**· 时间账户卡大数字下方辅助显示，帮用户心算"多少天"（较大数字用千分位） |
...
| `lifespanSubtitle(years, hoursK)` | lifespan 顶部卡副文案 | `45 年 · 47.3 万小时` | 顶部时间余额卡（§22.4） |
...
| 范围 | 格式 | 示例 |
|------|------|------|
| < 10,000 | 完整小时 + 千分位 | `552 小时`、`7,488 小时` |
| 10,000 – 999,999 | 一位小数万小时 | `1.8 万小时` |
| ≥ 1,000,000 | 整数万小时 | `100 万小时` |
...
| Scope | 时间范围 | lifespan 主数字 | 时间账户消耗层 | 存储层 |
|-------|----------|----------------|---------------|--------|
| 今生 | 从今天到预期寿命 / 对方时间窗口结束 | `约 N 周` | 保持原有 lifetime 计算 | 不变 |
| 今年 | 从今天到当年 12 月 31 日 | `N 周` | 用同一套公式计算年底前余额 | 不变 |

今年视角不显示 `2026.05.07 - 12.31` 这类日期范围；统一写「今年余额：N 周」。用户先在主页切到「今年」再进入任一时间账户时，详情页默认展示今年余额，并可在详情页切回「今生」。

### 时间公式页扣减节奏（V1.3.3）

| Dimension | 扣减节奏说明 |
|-----------|-------------|
| `parents` | 由 `visitsPerYear` 推导周期；例如每年 24 次 = 约每 2 周减少一次，每次扣 `hoursPerVisit`。 |
| `kids` | 按孩子年龄段连续下降；当前年龄段每周小时数持续折算到余额。 |
| `partner` | 按每天共处小时数连续下降。 |
| `sport` | 按年龄段每周运动小时数连续下降，跨年龄段时切换默认周时长。 |
| `create` | 专注创造期与自由创造期分别按每周小时数连续下降。 |
| `free` | 用剩余清醒时间减去其他时间账户估算占用，随其他账户与时间窗口连续变化。 |

扣减节奏不放在主页卡片同层展示；只出现在「时间窗口 → 时间公式 → 点击时间公式后的详情界面」里。
...
enum Formatter {
    ...
    static func lifespanSubtitle(years: Double, hoursK: Double) -> String // N 年 · N 万小时
...
所有 SwiftUI View 必须调用这些方法，不得自己拼 `"\(hours) 小时"` 或 `"约 \(n) 次..."`。

### 6 内置时间账户的副文案接口映射

| Dimension | 副文案文字 | 调用接口 |
|-----------|----------|---------|
| `lifespan` | `45 年 · 47.3 万小时` | `lifespanSubtitle(years:, hoursK:)` |
...
|-----------|---------------|--------|--------------|---------|
| `parents` | `552 小时` | `约 92 次见面` | 剩余年 × 见面频率 | Onboarding 勾选父母 + 详情页 UC-2.3 调 visitsPerYear/hoursPerVisit |
...
- 当 scope = `今年` 时，顶部卡标题写「今年余额」，主数字用剩余周数，副文案用剩余天数与小时辅助；下半「已存入」保持总存储层数字，不按今年过滤。
