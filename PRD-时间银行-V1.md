...
> Version: 1.2 | Date: 2026-04-19 | Author: Adamchen
>
> **V1.1 主要变更：** 引入"时间存储层"作为产品第二大核心机制，让"时间银行"的名字真正成立——不只是计数剩余，而是把有意义的时刻**永久封存**、**可增值**。
>
> **V1.2 主要变更：** 单位统一到小时（h）为主、"次/年/瞬间"为副；补充信息架构、Release Criteria、风险登记、空态清单、无障碍、i18n、Push/通知策略、评分策略、Changelog。
...
**时间银行**是一款 iOS App，让用户同时看到"人生中重要时间维度的剩余小时数"（消耗层）和"自己手动存入的有意义时刻"（存储层，带图文视频）。存储层时刻永久不被扣减，两层在每张维度卡片上形成对照。

**核心差异化**：市场上无产品同时做"家人剩余时间 + 日常体验觉知 + 时间存储 + Widget-first"。
...
- **社交/分享功能**：不做朋友圈分享、社区。V1 是个人工具，过早加社交会分散核心体验。
...
**验收标准：**
- 主页展示所有时间维度卡片（陪父母、陪孩子、运动、创造、陪伴侣、自由时间）
- 每张卡片显示核心数字 + 一句话描述
- 点击卡片进入详情页，展示计算逻辑和更多洞察
- 数字默认使用"第 N 次"正向叙事，可切换为"剩余 N 次"
...
| # | 功能 | 验收标准 |
|---|------|----------|
| P0-1 | 极简引导流程（3步） | 生日+性别 → 家人信息 → 完成。不超过 60 秒 |
| P0-2 | 主页时间维度卡片 | 展示 6 个内置维度：陪父母、陪孩子、运动、创造、陪伴侣、自由时间 |
| P0-3 | 卡片详情页 | 点击进入，展示完整数据、计算逻辑、洞察文案 |
| P0-4 | 参数可调 | 每个维度的核心参数可在详情页编辑 |
| P0-5 | "今日此刻"模块 | 展示 5+ 个日常时刻的"第 N 次"计数 |
| P0-6 | 锁屏 Widget | 小尺寸，每日轮换内容，温暖文案 |
| P0-7 | 本地数据存储 | 所有数据存本地，无需联网，无需注册 |
| **P0-8** | **【存储层】新增时刻（Deposit）** | **任何维度可点"+"存入时刻：标题、时长、文字、图片/视频（最多 9）、发生时间。原媒体存 App 沙盒。** |
| **P0-9** | **【存储层】维度时间线** | **卡片详情页下半部分展示该维度全部时刻，按发生时间倒序，显示缩略图 + 标题 + 时长。** |
| **P0-10** | **【存储层】时刻详情页** | **全屏多图轮播 / 视频播放 / 文字展开 / 元信息展示。可编辑、删除、换维度。** |
| **P0-11** | **【存储层】卡片底部"双数字"** | **每张维度卡片同时显示"消耗层剩余 + 存储层已存入"两个数字，形成对照。** |
| **P0-12** | **【存储层】数据导出** | **设置页可一键导出全部 ZIP（原媒体 + JSON/Markdown 索引），承诺用户数据可完整带走。** |

### P1 — Nice to Have（V1 有则更好）

| # | 功能 | 说明 |
|---|------|------|
| P1-1 | 自定义追踪项 | 用户创建自己的时间维度 |
| P1-2 | 桌面 Widget（中号） | Home Screen Widget，展示多维度概览 |
| P1-3 | 叙事模式切换 | "第 N 次" ↔ "剩余 N 次"全局切换 |
| P1-4 | 创造维度的子分类 | 允许"创造"下设多个子项（编程、写作等） |
| **P1-5** | **【存储层】时间资产总览页** | **独立二级页展示跨维度总时长、时刻数量、环形占比图** |
| **P1-6** | **【存储层】"X 年前的今天"回忆提醒** | **某条时刻在 1/2/5 年前的今天发生时，首页顶部软卡片提示** |
| **P1-7** | **【存储层】iCloud 同步（CloudKit）** | **SwiftData + CloudKit 同步私有数据库，用户换机不丢数据** |

### P2 — Future（V2+ 考虑）

| # | 功能 | 说明 |
|---|------|------|
| P2-1 | 轻量打卡 | "发生了就点一下"，自动记录 |
| P2-2 | 月度回顾 | 像一封温柔的信，展示时间分布 |
| P2-3 | Apple Watch | 表盘 Complication |
| P2-4 | StandBy 模式 | 充电时的大字时钟显示 |
| P2-5 | 时间快照对比 | 实际 vs 计划的时间花费对比 |
| **P2-6** | **【存储层】智能存入建议** | **基于近期相册连拍 + 日历事件，软提示用户存入一段时刻** |
| **P2-7** | **【存储层】年度回顾视频** | **年底自动生成一段"你的 X 年时间银行"视频，含精选瞬间** |
| **P2-8** | **【存储层】分享为图片卡** | **把一条时刻生成漂亮的分享卡（去社交化的纪念图）** |
...
| 阶段 | 时间 | 产出 |
|------|------|------|
| 产品定义 + Figma 设计 | 第 1-2 周 | PRD 定稿、3 个设计方向 mockup、选定最终方向 |
| SwiftUI 学习 + 核心开发 | 第 3-6 周 | App 主功能可用（引导 + 主页 + 详情 + 本地存储 + **存储层**） |
| Widget 开发 | 第 7-8 周 | 锁屏 + 桌面 Widget |
| 打磨 + TestFlight | 第 9-10 周 | 自用测试 + 邀请 5-10 人内测 |
| App Store 提交 | 第 11 周 | 提交审核 + 上线 |
...
│   ├── 时间维度卡片列表（6 内置 + N 自定义）
│   │   └── 点卡片 → [维度详情]
│   ├── 今日此刻模块（第 N 次洗澡 / 吃饭 / 早晨…）
│   │   └── 点条目 → [瞬间详情]（也是一种 Moment）
│   └── 浮动按钮「+」→ [新增时刻表单]
│
├── [维度详情页]
│   ├── 顶部 — 消耗层（剩余小时 + 副文案）
...
### 功能完整性
- [ ] P0-1 ~ P0-12 全部完成并在真机上可用
- [ ] 自动化 UI Test 覆盖三条主路径：引导、新增时刻、查看时刻
- [ ] Widget 在真机锁屏、真机桌面、StandBy 三种场景下可显示
- [ ] iCloud 未登录时 App 能正常运行（降级为纯本地）

### 数据完整性
- [ ] 任何时刻的保存都是原子的：要么全部写入（元数据 + 媒体文件），要么全部回滚
- [ ] App 崩溃或中途杀进程不会留下孤儿文件（启动时扫描并清理）
- [ ] 一键导出的 ZIP 解压后结构清晰可用
...
- [ ] Dark Mode / Light Mode 全部场景走查一遍
- [ ] Dynamic Type 最小档到最大档不截断关键内容
...
| R9 | Adam 本人 SwiftUI 经验不足拖延 timeline | 高 | 中 | 保留 25% 时间缓冲；可选：找 Codex / GPT 协助代码 review |
...
**V1 只保留 1 种通知**：**每日温暖提醒**（可关闭）

- 触达频率：每天 1 次，默认上午 9:00，用户可关
- 文案池（从 30+ 条随机抽取）：
  - "你今天又多见了一次太阳升起。"
  - "现在这一刻，是你以后回想起的'过去'。"
  - "过去 24 小时有什么值得存入时间银行的吗？"
  - "你和爸妈上次见面是 X 天前。"（条件触发）
- 引导点击 → 进入"新增时刻"表单（预填"今天"）

**V1 明确不做的通知**：
- ❌ 每日任务打卡提醒（违反产品哲学"不是 KPI"）
- ❌ "你已经 X 天没陪爸妈了"（会触发焦虑，违反调性）
- ❌ 推广/营销通知
- ❌ 锁屏弹窗
...
| V1.1 | 2026-04-19 | 引入存储层（图文视频沉淀）、双层模型、数据模型、P0-8~P0-12 |
