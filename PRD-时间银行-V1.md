...
**时间银行**是一款 iOS App，让用户同时看到"人生中重要关系维度的剩余小时数"（消耗层）和"自己手动存入的有意义时刻"（存储层，带图文）。存储层时刻永久不被扣减，两层在每张维度卡片上形成对照。

**差异化（克制版）**：我们没看到主流产品把"关系维度剩余时间 + 存入式记忆账户 + local-first 隐私承诺 + Widget 触达"这四个要素整合在一个体验里。单点都有人做（WeCroak 做死亡提醒、Your Time 做剩余人生、Day One 做记录容器），但这套组合是新的。moat 在组合，不在单点。
...
主页                · 6 内置维度卡片（visible 由 onboarding 决定）
                    · 每卡双数字（消耗 vs 存储）
                    · 顶部总账户卡片
维度详情             · 参数编辑 + 维度级时间线
...
Memorial mode       · 任何关系维度可进入（纪念模式，只保留存储层）
...
**同时还有第二层问题——**
相册里有很多珍贵的照片，但它们被埋在"按时间倒序"的海量流水里，没有被赋予**"这一小时属于陪爸妈"**这样的上下文。用户知道这些时刻重要，但缺乏一个容器把它们**按关系/维度**结构化地沉淀下来，导致这些瞬间从未真正"被存进去"，只是在相册里慢慢被新照片淹没。
...
- 消耗层给出"稀缺感"，存储层给出"积累感"
- 每存入一个时刻，该维度的"已存入"数字变大，给用户一种"时间资产在复利增长"的正反馈
...
**次要用户：** 20-30 岁年轻人，尚未组建家庭，但想追踪个人成长维度（运动、创造、学习）。
...
- **明确支持单身、丁克、无父母、独处人群**：Step 2 用 chip 多选"你和谁共度着时间？"，未勾选的维度不展示、不占位、不提示
- 允许 Step 2 一个都不勾（走自由维度 + 日常时刻即可）
- 所有字段默认值由系统根据年龄自动计算
...
As a 用户, I want to 一眼看到我在各个人生维度的剩余时间, so that 我能直观感受到什么是真正重要的。

**验收标准：**
- 主页**仅展示用户在 onboarding 里勾选了的关系维度**（父母/孩子/伴侣）+ 通用维度（运动/创造/自由）
- 内置维度在数据库中始终存在，但通过 `Dimension.status = visible | hidden` 控制展示
- 用户可后续在设置里把 hidden 的维度转为 visible
- 每张卡片显示双数字（消耗 + 存储）+ 副文案
...
As a 用户, I want to 创建属于我自己的时间维度, so that 我能追踪对我个人有意义的事情。
...
As a 用户, I want to 把一段有意义的时间手动"存入"对应的时间维度, so that 这段时间被永久记录下来，不会像消耗层那样随着日子流逝而扣减。
...
As a 用户, I want to 点开任一维度看到我存入的所有时刻, so that 我能一次性回顾我在这个关系/主题下积累的所有珍贵片段。

**验收标准：**
- 每个维度详情页下半部分是该维度的"时间线"
- 每条时刻展示：第一张图缩略图 / 标题 / 发生时间 / 时长 / 文字首句
- 按发生时间倒序（默认），可切换为正序
- 长按一条时刻可进入多选模式，批量删除 / 导出 / 换维度
...
**验收标准：**
- 主页顶部或二级页展示一个"总账户"视图
- 展示：累计存入时长（如 "128 小时"）、时刻数量、各维度占比（环形图）
...
**验收标准：**
- 每张维度卡片同时显示两组数字（按 §21 Formatter Matrix 格式）：
  - 消耗层：`552h · 约 92 次见面`
  - 存储层：`56h · 12 个瞬间`
...
As a 用户, I want to 在经历失去后，温柔地把这个维度转为"只纪念，不计算", so that 产品不会继续在我面前显示冷冰冰的"还剩 X 小时"。

**适用范围**：任何关系类型维度 —— parents / kids / partner / 以及 V1.1 后的自定义关系维度（如"爷爷"、宠物名）。不仅限于 parents。
...
- 维度进入 memorial 后：
  ...
  - 卡片下方显示 chip："致爸爸"
  - 维度色保留（不变灰、不变冷）
  - Widget 上该维度不再作为"剩余倒计时"类内容轮播，只作为"回忆触达"出现（若开启）
...
**验收标准：**
- 设置页提供"导出全部数据"入口
- 导出为一个 ZIP，内含：所有原图/原视频 + 按维度分文件夹 + 一个 JSON 或 Markdown 结构化索引
...
| # | 功能 | 验收标准 |
|---|------|----------|
| P0-1 | 引导流程（4 步 + Pre-permission） | 生日 → 关系 chip 多选 → 条件化详情 → 完成。**支持 0 勾选**（单身 / 无父母 / 只用自我维度）。总耗时 ≤ 60s |
| P0-2 | 主页 · 双数字维度卡片 | 只展示 onboarding 勾选了的内置维度 + 通用维度（运动/创造/自由）。每张卡片同时显示"消耗层剩余 h + 存储层已存入 h + 副文案"，格式按 §21 Formatter Matrix |
| P0-3 | 主页 · 顶部总账户卡片 | 累计存入小时数、瞬间数、跨几个维度 |
| P0-4 | 维度详情页 | 上半 = 消耗层大数字 + 副文案 + 参数编辑卡；下半 = 该维度时间线 |
| P0-5 | 维度参数编辑 | 每个内置维度的核心参数（频率、每次时长、预期寿命等）可调；恢复默认值 |
| P0-6 | 存储层 · 新增时刻 | 字段：维度、发生时间、时长（可空）、标题、长文本、图片（最多 9 张，只从 PhotosPicker 选，不做 App 内拍照/拍视频）。**V1 允许从相册选视频**，但 V1 不提供 App 内相机录视频。保存走**强原子性事务** |
| P0-7 | 存储层 · 维度时间线 | 维度详情页下半部分展示全部时刻，倒序，缩略图+标题+时长+笔记首句 |
| P0-8 | 存储层 · 时刻详情页 | 全屏媒体轮播（图+视频播放）、文字展开、Chip、底部动作：编辑 / 换维度 / 删除。**V1 不含"分享卡"** |
| P0-9 | 时刻 CRUD 完整闭环 | 编辑所有字段、换维度、删除走延迟 5s + Undo Toast 协议 |
| P0-10 | 批量选择 + 批量换维度 + 批量删除 | 时间线长按进入批量模式 |
| P0-11 | 账户 Tab | 独立二级 Tab：总账户数字、环形占比图、年度柱状视图、"X 年前的今天"卡片 |
| P0-12 | Memorial Mode | 任何关系维度（parents/kids/partner/custom）可标记为"纪念模式"：不再计算消耗、只保留存储层；视觉保持温暖（不变灰）|
| P0-13 | 锁屏 Widget（1 种 family） | 小号锁屏 Widget，显示维度数字或温暖文案；从 App 生成的预计算快照读取（走 App Group）|
...
| # | 功能 | 说明 |
|---|------|------|
| P1-1 | "今日此刻"模块 | 第 N 次洗澡 / 吃饭 / 早晨等；走隐藏维度 `daily` |
...
- **总账户（主页顶部）**："已存入 128 小时 · 27 个瞬间 · 跨 5 个维度"
...
| 字段 | 类型 | 说明 |
|------|------|------|
| id | String | "parents" / "kids" / "create" / "partner" / "sport" / "free" 或 UUID（自定义维度）|
| name | String | 展示名 |
...
@Model final class Dimension {
    ...
    var sortIndex: Int
    var params: Data                             // JSON-encoded 维度参数（按 kind 不同）
    var createdAt: Date
    var updatedAt: Date
}
...
@Model final class Moment {
    ...
    var originDimensionId: String?               // 曾经的维度（因迁移到 other 时记录，方便"找回"）
    var title: String?
    var note: String                             // 默认 ""
    var happenedAt: Date
...
- **共享数据策略**：
  ...
  - Snapshot 字段：每个 visible 维度的 `{id, name, iconKey, colorKey, consumeHours, storedHours, momentCount, lastMoment}` + `topText`（当前 Widget 文案池随机抽的一条）
...
| Onboarding + 主页 + 维度详情 | 第 5-7 周 | 可用的 UI 闭环（能 onboarding → 看主页 → 看详情 → 存一条 Moment） | 最容易超时的阶段；一旦卡住立即向外求助 |
...
| 维度详情 · 该维度 0 时刻 | 时间线区域 | "这里还没有被存下的时刻。下次和 {爸妈} 见面后，记得回来存进时间银行。" | 小插画，不是"暂无数据" |
| 账户 Tab · 没有任何数据 | 中间 | "存入第一个时刻，就能在这里看到你的时间资产图。" | — |
| 搜索 · 无结果 | 搜索框下 | "没找到相关的时刻。" | — |
| 自定义维度 · 0 个自定义 | 维度管理页 | "你可以追踪任何对你重要的时间，比如阅读、旅行、冥想。" + CTA | — |
...
- VoiceOver 标签：每张维度卡片朗读为 "{维度名}，还能存入 X 小时，已存入 Y 小时"
...
**V2**：可能加"时间银行 · 协作版"（家庭成员共享某些维度）——定价再议
...
| V1.0 | 2026-04-01 | 初版 PRD：消耗层 6 维度、日常时刻、Widget-first |
...
| `occurrenceCount(n, noun)` | 次数（消耗层用）| `约 92 次见面`、`约 40 年`、`每周约 5h` | 维度卡副文案 |
