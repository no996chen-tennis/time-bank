...
> 本文件是 Claude Code Project 会自动加载的项目索引。当前 PRD 版本：**V1.3**（2026-04-22）
...
- **时间可以被"存起来"**：有意义的时刻（带图文）沉淀到对应维度，永不被扣减 —— 这是产品叫"时间银行"的根本原因
...
两层在每张维度卡片同时出现："还能存入 552h · 约 92 次见面" vs "已存入 56h · 12 个瞬间"。

## V1 Scope（12 周 timeline · 2026-04-22 定稿）

**做：** Onboarding（4 步 + chip 多选，支持单身/无父母/丁克） · 6 内置维度双数字卡片 · 时刻 CRUD（图文，只从相册选）· Memorial Mode（任何关系维度）· 账户 Tab · 1 个锁屏 Widget · Raw ZIP 导出 · **Light Mode Only**

**推迟到 V1.1+：** 今日此刻 · 自定义维度 CRUD · 分享卡 · 视频拍摄 · 保存到相册 · 桌面 Widget · StandBy · **Dark Mode** · 多格式导出 · 从 ZIP 恢复 · CloudKit 同步（V1.5 两阶段做）
...
- `PRD-时间银行-V1.md` — **V1.3** 产品需求文档（含 §7.6 Authoritative Schema + §21 Formatter Matrix）
- `Use-Cases-详尽交互.md` — **V1.1** 所有模块 CRUD 逐步交互（工程交付真相源）
...
- `文案系统.md` — **V1.1** Tone of voice + 文案库 + Widget/通知双池 + Formatter Matrix
...
- `设计-最终交付prompt-自包含版.md` — **最新 · 发给任何无上下文对话框用**（产品背景 + 14 屏清单 + 5 Part 全部内嵌，不依赖 GitHub fetch）
- `设计-最终交付prompt-Headspace版.md` — 依赖 GitHub raw 链接版本（如果对话框能 fetch URL 则用这份更简洁）
- `设计提示词-浅色5风格.md` — 浅色 5 风格探索提示词（风格 A 已被最终交付 prompt 取代，其他 4 个保留备用）
- `设计提示词-5风格.md` — ⚠️ 已归档，原 5 方向（含深色候选），保留作历史参考
